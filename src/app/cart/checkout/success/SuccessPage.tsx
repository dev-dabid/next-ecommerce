...
export default function Success({ order }: SuccessPageProps) {
  ...
  const displaySubTotal = `$${formattedPrice(calculatedSubTotalCents)}`;
  const displayShipping = `${order.shippingType}`;
  const displayTotal = `${formattedPrice(order.totalPrice)}`;
  const displayRecipient = `${order.firstName} ${order.lastName}`;
  const displayAddress = `${order.streetAddress}, ${order.barangay}, ${order.city}, ${order.province}, ${order.zipCode}`;
          ...
          <div className="flex-1 bg-white flex flex-col rounded md:rounded-3xl md:p-10 lg:p-10">
            ...
            <div className="flex flex-col p-5 md:p-0">
              <div className="flex justify-between mb-3">
                ...
                <p className="font-semibold">{displaySubTotal}</p>
              </div>
              <div className="flex justify-between mb-8">
                <p className="text-gray-400">Shipping</p>
                <p className="font-semibold">{displayShipping}</p>
              </div>
            ...
            <div className="flex w-full flex-col md:justify-between md:flex-row lg:flex-col gap-5 h-full">
              ...
              <div className="md:col-span-2  bg-white rounded md:rounded-3xl p-5 md:p-10 h-full flex-1">
                ...
                <div className="mb-5">
                  ...
                  <p className="text-lg font-bold">{displayRecipient}</p>
                </div>
                <div>
                  <p className="text-gray-600 font-semibold">ADDRESS</p>
                  <p className="text-md font-semibold">{displayAddress}</p>
                </div>
              </div>
            </div>
